...
impl Game {
    pub fn dump(&self, file: &mut std::fs::File) {
        ...
        let pieces: Vec<u8> = self.board.pieces.iter().map(|x|x.to_le_bytes()).flatten().collect();

        file.write_all(&pieces).unwrap();

        file.write_all(&self.board.stm_ep.to_le_bytes()).unwrap();
        file.write_all(&self.board.halfmove_clock.to_le_bytes()).unwrap();
...
impl PackedBoard {
    pub fn from_position(pos: &noggin::Position, result: u8) -> Self {
        ...
        let pieces: Vec<u8> = pos.board.iter().enumerate().map(|(sq, &p)|{
            let side = if ((1u64 << sq) & black_occ) != 0 {noggin::Side::Black} else {noggin::Side::White};
