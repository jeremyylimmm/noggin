...
impl Accumulator {
    ...
    pub fn new(bb: &[u64]) -> Self {
        ...
        data[0..HL0_SIZE / 2].copy_from_slice(&B0);
        data[HL0_SIZE / 2..].copy_from_slice(&B0);
...
fn screlu(x: i16) -> i32 {
    let y = x.clamp(0, QA as i16) as i32;
    y * y
}
...
fn screlu_y0(y0: &[i16; HL0_SIZE]) -> [i32;HL0_SIZE] {
    std::array::from_fn(|i|screlu(y0[i]))
}
...
pub fn compute(bb: &[u64]) -> i32 {
    ...
    y0[0..HL0_SIZE / 2].copy_from_slice(&B0);
    y0[HL0_SIZE / 2..].copy_from_slice(&B0);
