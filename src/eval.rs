...
pub const MG_VALUE: [Score; 6] = [82, 337, 365, 477, 1025, 0];
pub const EG_VALUE: [Score; 6] = [94, 281, 297, 512, 936, 0];

/* piece/sq tables */
/* values from Rofchade: httpVkk://www.talkchess.com/forum3/viewtopic.php?f=2&t=68311&start=19 */
...
pub fn compute(pos: &Position) -> (Score, Score, i32) {
    ...
    let black_occ = pos.side_occ(Side::Black);

    /* evaluate each piece */
    for sq in 0..64 {
        let col = if Sq(sq).bb() & black_occ != 0 {
            Side::Black
        } else {
            Side::White
        };

        let sq_rel = Sq(if col == Side::Black { sq } else { sq ^ 56 });

        if let Some(pc) = pos.board[Sq(sq)] {
            mg[col.id()] += MG_PESTO_TABLE[pc.id()][sq_rel.id()] + MG_VALUE[pc.id()];
            eg[col.id()] += EG_PESTO_TABLE[pc.id()][sq_rel.id()] + EG_VALUE[pc.id()];
            game_phase += GAMEPHASE_INC[pc.id()];
        }
    }
