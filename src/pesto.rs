...
pub fn eval_internal(bb: &[u64], board: &[Piece]) -> (i32, i32, i32) {
    ...
    //let white_bb = bb[..6].iter().fold(0, |acc, x|acc|x);
    let black_bb = bb[6..].iter().fold(0, |acc, x| acc | x);

    /* evaluate each piece */
    for sq in 0..64 {
        let side = if black_bb & (1u64 << sq) != 0 { 1 } else { 0 };

        let sq_idx = if side == 0 { sq ^ 56 } else { sq };

        let piece = board[sq];

        if let Some(pc) = piece.id() {
            mg[side] += MG_PESTO_TABLE[pc][sq_idx] + MG_VALUE[pc];
            eg[side] += EG_PESTO_TABLE[pc][sq_idx] + EG_VALUE[pc];
